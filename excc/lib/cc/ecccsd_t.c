...
double _ecccsd_t_get_energy(double *w, double *v, double *mo_energy, bool *t3ec_mask,
                            int nocc, int ncas, int nco, int ncv, int nc, int a, int b, int c, double fac)
{
        int i, j, k, n;
        ...
        for (j = 0; j < nocc; j++) {
        for (k = 0; k < nocc; k++, n++) {
                if (i >= nc && j >= nc && k >= nc && a < ncv && b < ncv && c < ncv)
                    if (t3ec_mask[(((((i-nc)*nco+j-nc)*nco+k-nc)*ncv+a)*ncv+b)*ncv+c])
                        continue;
...
static double _get_ec_energy_baa(double *z0, double *z1, double *w0, double *w1,
                              double *mo_ea, double *mo_eb, bool *t3ec_mask,
                              int nocca, int noccb, int ncas, int nco, int ncv, int nc,
                              int a, int b, int c, double fac)
{
        int noo = nocca * nocca;
        int i, j, k;
...
static double contract6_aaa(int nocc, int nvir, int ncas, int nco, int ncv, int nc,
                            int a, int b, int c,
                            double *mo_energy, double *t1T, double *t2T,
                            int nirrep, int *o_ir_loc, int *v_ir_loc,
                            int *oo_ir_loc, int *orbsym, double *fvo,
                            double *vooo, bool *t3ec_mask, double *cache1,
                            void **cache, int *permute_idx)
{
        ...
        double et;
        if (a == c) {
                et = _ecccsd_t_get_energy(w0, z0, mo_energy, t3ec_mask, nocc, ncas, nco, ncv, nc, a, b, c, 1./6);
        } else if (a == b || b == c) {
                et = _ecccsd_t_get_energy(w0, z0, mo_energy, t3ec_mask, nocc, ncas, nco, ncv, nc, a, b, c, .5);
        } else {
                et = _ecccsd_t_get_energy(w0, z0, mo_energy, t3ec_mask, nocc, ncas, nco, ncv, nc, a, b, c, 1.);
        }
        return et;
}
...
void CCecccsd_t_aaa(double complex *e_tot,
                    double *mo_energy, double *t1T, double *t2T,
                    double *vooo, double *fvo, bool *t3ec_mask,
                    int nocc, int nvir, int ncas, int nco, int ncv, int nc,
                    int a0, int a1, int b0, int b1,
                    int nirrep, int *o_ir_loc, int *v_ir_loc,
...
#pragma omp parallel default(none) \
        shared(njobs, nocc, nvir, ncas, nco, ncv, nc, mo_energy, t3ec_mask, t1T, t2T, \
               nirrep, o_ir_loc, v_ir_loc, oo_ir_loc, orbsym, vooo, fvohalf, \
               jobs, e_tot, permute_idx)
{
        int a, b, c;
        size_t k;
...
#pragma omp for schedule (dynamic, 4)
        for (k = 0; k < njobs; k++) {
                ...
                b = jobs[k].b;
                c = jobs[k].c;
                e += contract6_aaa(nocc, nvir, ncas, nco, ncv, nc, a, b, c,
                                   mo_energy, t1T, t2T,
                                   nirrep, o_ir_loc, v_ir_loc, oo_ir_loc, orbsym,
                                   fvohalf, vooo, t3ec_mask, cache1, jobs[k].cache,
                                   permute_idx);
        }
        free(cache1);
#pragma omp critical
...
static double contract6_baa(int nocca, int noccb, int nvira, int nvirb,
                            int ncas, int nco, int ncv, int nc,
                            int a, int b, int c, double **vs_ts, 
                            bool *t3ec_mask, void **cache, double *cache1)
{
        ...
        double et;
        if (b == c) {
                et = _get_ec_energy_baa(z0, z1, w0, w1, mo_ea, mo_eb, t3ec_mask, nocca, noccb, ncas, nco, ncv, nc, a, b, c, .5);
        } else {
                et = _get_ec_energy_baa(z0, z1, w0, w1, mo_ea, mo_eb, t3ec_mask, nocca, noccb, ncas, nco, ncv, nc, a, b, c, 1.);
        }
        return et;
}
...
void CCecccsd_t_baa(double complex *e_tot,
                   ...
                   double *fvo, double *fVO, bool *t3ec_mask,
                   int nocca, int noccb, int nvira, int nvirb,
                   int ncas, int nco, int ncv, int nc,
                   int a0, int a1, int b0, int b1,
                   void *cache_row_a, void *cache_col_a,
...
#pragma omp parallel default(none) \
        shared(njobs, nocca, noccb, nvira, nvirb, ncas, nco, ncv, nc, \
               t3ec_mask, vs_ts, jobs, e_tot)
{
        int a, b, c;
        size_t k;
...
#pragma omp for schedule (dynamic, 4)
        for (k = 0; k < njobs; k++) {
                ...
                b = jobs[k].b;
                c = jobs[k].c;
                e += contract6_baa(nocca, noccb, nvira, nvirb, ncas, nco, ncv, nc,
                                   a, b, c, vs_ts, t3ec_mask, jobs[k].cache, cache1);
        }
        free(cache1);
#pragma omp critical
        *e_tot += e;
